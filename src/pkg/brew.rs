...
/// Render a snapshot Brewfile. `mas`/`vscode` are apply-only (outside the diff
/// scope), so they are left to the manifest rather than guessed at here.
fn render_brewfile(taps: &[String], brews: &[String], casks: &[String]) -> String {
    let mut out = String::from("# Generated by `macboot pkg dump`.\n");
    out.push_str("# Install with: brew bundle --file=packages/brew/Brewfile\n");
    for (keyword, values) in [("tap", taps), ("brew", brews), ("cask", casks)] {
        if values.is_empty() {
            continue;
        }
...
impl Provider for BrewProvider {
    ...
    /// Homebrew's own format, so the dump is directly usable as
...
mod tests {
    ...
    #[test]
    fn brewfile_dump_is_grouped_sorted_and_quoted() {
        let got = render_brewfile(
            &v(&["homebrew/cask-fonts"]),
            &v(&["ripgrep", "bat"]),
            &v(&["ghostty"]),
        );
    ...
    #[test]
    fn brewfile_dump_omits_empty_sections() {
        let got = render_brewfile(&[], &v(&["fd"]), &[]);
        assert!(!got.contains("tap "), "{got}");
        assert!(!got.contains("cask "), "{got}");
        assert!(got.contains(r#"brew "fd""#), "{got}");
    }
}
